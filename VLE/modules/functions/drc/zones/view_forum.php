...
<?php
if(isset($_GET['id'])){
$qry = $db->query("SELECT t.*,u.name FROM topics t LEFT join users u on u.id = t.user_id where t.id= ".$_GET['id']);
foreach($qry->fetch_array() as $k => $val){
	$$k=$val;
}
$comments = $db->query("SELECT c.*,u.name,u.username FROM comments c 
	LEFT join users u on u.id = c.user_id 
	where c.topic_id= ".$_GET['id']."
	order by unix_timestamp(c.date_created) asc");
$com_arr= array();
while($row= $comments->fetch_assoc()){
	$com_arr[] = $row;
}
$replies = $db->query("SELECT r.*,u.name,u.username FROM replies r LEFT join users u on u.id = r.user_id where r.comment_id in (SELECT id FROM comments where topic_id= ".$_GET['id'].") order by unix_timestamp(r.date_created) asc");
$rep_arr= array();
while($row= $replies->fetch_assoc()){
	$rep_arr[$row['comment_id']][] = $row;
}
if($user_id != $_SESSION['id']){
$chk = $db->query("SELECT * FROM forum_views where  topic_id=$id and user_id='{$_SESSION['id']}' ")->num_rows;
if($chk <= 0){
	$db->query("INSERT INTO forum_views set  topic_id=$id , user_id='{$_SESSION['id']}' ");
}
}
$view = $db->query("SELECT * FROM forum_views where topic_id=$id")->num_rows;
$tags = array();
if(!empty($category_ids)){
$tag = $category_ids;
// $db->query("SELECT * FROM categories where id in ($category_ids) order by name asc");
    				...
    				<div>
    					...
    					<div class="col-lg-8 offset-lg-2 replies">
    						...
    						<div class="form-group ty-compact-list">
			    				...
			    				<p class="mb-0"><large><b><?php echo $rep['name'] ?></b></large>  </p>
