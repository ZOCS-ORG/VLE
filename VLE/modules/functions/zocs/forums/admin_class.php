...
Class Action {
	...
	function update_account(){
		...
		if(!empty($password))
		$data .= ", password = '".md5($password)."' ";
		$chk = $this->db->query("SELECT * FROM users where username = '$email' and id != '{$_SESSION['id']}' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("UPDATE users set $data where id = '{$_SESSION['id']}' ");
		if($save){
			$data = '';
			foreach($_POST as $k => $v){
				if($k =='password')
	...
	function search(){
		...
		while($row= $ts->fetch_assoc()):
			$tsearch .=" or concat('[',REPLACE(t.audience,',','],['),']') like '%[{$row['id']}]%' ";
		endwhile;
		...
		while($row= $topic->fetch_assoc()):
	        ...
	        $row['tags'] = array();
	        foreach(explode(",",$row['audience']) as $cat):
	        	$row['tags'][]= $tags[$cat];
			endforeach;
